...
export function AuthForm() {
  ...
  const handleFirebaseLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Send to our backend for verification + local user creation
  ...
  const handleGoogleSignIn = async () => {
    ...
    setLoading(true);
    try {
      const { auth: firebaseAuth, googleProvider } = await import('@/lib/firebase/client');
      if (!firebaseAuth) throw new Error('Firebase not configured');
      const result = await signInWithPopup(firebaseAuth, googleProvider);
      const idToken = await result.user.getIdToken();
      // Send to backend for verification + local user record
      const res = await fetch('/api/auth/login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ idToken }),
  ...
  const handleSubmit = isFirebaseConfigured
    ? (mode === 'login' ? handleFirebaseLogin : handleFirebaseRegister)
    : handleLocalAuth;
      ...
      <div className="w-full max-w-md space-y-8">
        ...
        <Card className="shadow-xl border-0 shadow-black/5">
          <CardHeader className="text-center pb-2">
            ...
            <CardDescription>
              {mode === 'login' ? 'Sign in to your Pulse AI account' : 'Get started with a free account'}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {/* Google Sign-In Button (Firebase mode) */}
            ...
            <div className="mt-4 text-center text-sm text-muted-foreground">
              {mode === 'login' ? (
                <>Don&apos;t have an account?{' '}
                  <button onClick={() => { setMode('register'); setError(''); }} className="text-primary font-medium hover:underline">Sign up</button>
                </>
              ) : (
        ...
        <p className="text-center text-xs text-muted-foreground">
          {isFirebaseConfigured ? 'Secured with Firebase Authentication' : 'Audit websites with deterministic analysis + AI-powered insights.'}
        </p>
      </div>
    </div>
  );
